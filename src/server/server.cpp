...
EchoServer::EchoServer() noexcept
    : max_conn_(DEF_MAX_CONN), master_socket_(0), connections_count_(0) {
  address_.sin_port = htons(DEF_PORT);
  this->address_init();
}

EchoServer::EchoServer(const int &port, const int &max_conn) noexcept
    : max_conn_(max_conn), master_socket_(0), connections_count_(0) {
...
void EchoServer::set_master_socket() {
  ...
  int setsock_code = setsockopt(master_socket_, SOL_SOCKET, SO_REUSEADDR,
                                (char *)&opt, sizeof(opt));
  if (setsock_code < 0) {
    throw ServerBindSocketError(
        "Set multiple connections to master socket failed");
  }
}

void EchoServer::bind_master_socket() {
  int bind_code =
      bind(master_socket_, (struct sockaddr *)&address_, sizeof(address_));
  if (bind_code < 0) {
    throw ServerBindPortIPError(
        "Bind master socket to IP:port failed; port used by another process ?");
  }
}
...
void EchoServer::main_server_loop(int *addrlen,
  ...
  while (true) {
    ...
    if (FD_ISSET(master_socket_, &readfds)) {
      new_socket = accept(master_socket_, (struct sockaddr *)&address_,
                          (socklen_t *)addrlen);
      if (new_socket < 0) {
      ...
      printf("New connection , socket fd is %d , ip is : %s , port : %d \n",
             new_socket, inet_ntoa(address_.sin_addr),
    ...
    for (int i = 0; i < max_conn_; i++) {
      ...
      if (FD_ISSET(sd, &readfds)) {
        ...
        if (valread < 1) {
          ...
          // SIGPIPE
          // if (valread == -1) {
          //   std::cout << "149: "<< errno << " valread is " << valread <<
          //   std::endl;
          // }
        } else {
          buffer[valread] = '\0';
          ...
          int send_res =
              send(sd, return_str.c_str(), strlen(return_str.c_str()), 0);
          if (send_res < 0) {
